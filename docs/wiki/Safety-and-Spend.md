...
These rules are product DNA — not optional polish.

## BYOK

Provider credentials live in the **OS vault** (Desktop → **Keys**). Never commit `.env` secrets, PEMs, or credential JSON.

## Spend honesty (H1)

- Session / daily caps require honest **$/MTok** (or explicit unmetered / `ADE_ALLOW_UNPRICED`)  
- Reserves use estimated message size — not the full context window billed as if filled  
- Trust shows **used / reserved / remaining**; ledger shows reserved − actual (Δ)  
...
| Mode | Meaning |
|------|---------|
| **Suggest** | Planner / inspect — no write leases |
...
Handoff capsules + thrift resume — Desktop **Continue** or `ade handoff resume`. Prefer host `next_safe` over paste theater.
