---
layout: default
title: Architecture decision records
---

# Architecture decision records

ADRs for ADE. Format: `DEC-A-NNN-slug.md`.

**Wiki home:** [ADE Wiki](../)

| ID | Title | Status |
|----|-------|--------|
| [DEC-A-010](DEC-A-010-multi-host-agent-os.html) | Multi-host Agent OS (Zed + Desktop) | Accepted (amended by 013) |
| [DEC-A-011](DEC-A-011-repo-layout.html) | Repository layout (evolutionary) | Accepted |
| [DEC-A-012](DEC-A-012-acp-first-openvsx-companion.html) | ACP first; Open VSX companion | **Superseded by 013** |
| [DEC-A-013](DEC-A-013-zed-only-fork-ladder.html) | Zed-only hosts; gated fork ladder | Accepted |
| [DEC-A-014](DEC-A-014-harness-first-zed-optional.html) | Harness-first; Zed host optional | Accepted |
| [DEC-A-015](DEC-A-015-z2-fork-ladder-review.html) | Z2 fork ladder review | Accepted |

Canonical layout: [REPO_LAYOUT](../architecture/REPO_LAYOUT.html).
